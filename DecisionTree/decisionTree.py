...
def createTree(*dataSet):
    ...
    '''
    #设置threshold，“5.3.1 ID3算法”第4步提到需要将信息增益与阈值threshold比较，若小于则
    #直接处理后返回T
    ...
    classSet = {i for i in trainLabelList}
    ...
    Ag, thresholdGet = calcBestFeature(trainDataList, trainLabelList)

    #如果Ag的信息增益比小于阈值threshold，则置T为单节点树，并将D中实例数最大的类Ck
    #作为该节点的类，返回T
    if thresholdGet < threshold:
        return majorClass(trainLabelList)
...
if __name__ == '__main__':
    ...
    tree = createTree((Train[:,:-1].tolist(), Train[:,-1].tolist()))
    print('tree is:', tree)

    #测试准确率
    print('start test')
    accur = model_test(Test[:,:-1],Test[:,-1], tree)
