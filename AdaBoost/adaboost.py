    ...
    for line in fr:
        ...
        for feat in feats[:-1]:
            if float(feat) >= 0.5:
                ...
                tmpList.append(0)
        tmpList.append(-1 if int(feats[-1]) >= 5 else 1)
...
if __name__ == '__main__':
    ...
    tree = createBosstingTree(Train[:, :-1], Train[:, -1], 4)

    #测试
    print('start to test')
    accuracy = model_test(Test[:, :-1], Test[:, -1], tree)
